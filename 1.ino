...
int tar_x[] = {0,20,20, 0,0};
int tar_y[] = {0,0,20,20,0};

int iterations=3;     // how many time we need to repeate the waypoints 
int mission=0;        // Counting the number of rotations (from start to end
//
//int tar_th[] = {90};  
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(R_encoder), doREncoder, CHANGE);
  attachInterrupt(digitalPinToInterrupt(L_encoder), doLEncoder, CHANGE);
}

void loop() {

if (mission <iterations){
...
//    Serial.print("Khalil kahta iski value ni aati : ");
//    Serial.println(T_th);
